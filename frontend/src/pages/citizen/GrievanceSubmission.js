...
const GrievanceSubmission = () => {
  ...
  const validateForm = () => {
    if (!form.Category) {
      ...
      showMessagePopup('Help us route your grievance correctly by selecting an appropriate category.');
      return false;
    }
    if (!form.title) {
      setErrorField('title');
      titleRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      showMessagePopup('Adding a clear title will help us understand your grievance better.');
      return false;
    }
    if (!form.description) {
      setErrorField('description');
      descriptionRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      showMessagePopup('A detailed description will help us address your concerns more effectively.');
      return false;
    }
    if (!form.street) {
      setErrorField('street');
      streetRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      showMessagePopup('We need your street address to locate the issue accurately.');
      return false;
    }
    if (!form.city) {
      setErrorField('city');
      cityRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      showMessagePopup('Including your city helps us identify the correct jurisdiction.');
      return false;
    }
    if (!form.district) {
      setErrorField('district');
      districtRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      showMessagePopup('Your district information helps us coordinate with the right authorities.');
      return false;
    }
    if (!form.state) {
      setErrorField('state');
      stateRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      showMessagePopup('Knowing your state ensures proper routing of your grievance.');
      return false;
    }
    if (!form.postalCode || form.postalCode.length !== 6) {
      ...
      showMessagePopup('A valid 6-digit postal code helps us pinpoint your location precisely.');
      return false;
    }
    setErrorField('');
    return true;
          ...
          <div style={{
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <textarea
                ...
                style={{ ...inputStyle, width: '95%', minHeight: 120, resize: 'vertical' }}
